...
obj = "PHEME_" # choose dataset, you can choose either "Twitter15" or "Twitter16"
fold = "3" # fold index, choose from 0-4
#tag = "_u2b"
vocabulary_size = 164000
hidden_dim = 100
Nclass = 4
Nepoch = 600
lr = 0.005
...
treePath = '../resource/abcd_TD.txt' 

trainPath = "../nfold/RNNtrainSet_"+obj+str(fold)+".txt" 
testPath = "../nfold/RNNtestSet_"+obj+str(fold)+".txt"
labelPath = "../resource/label.txt"
...
def loadLabel(label, l1, l2, l3, l4):
    labelset_nonR, labelset_f, labelset_t, labelset_u = ['news', 'non-rumour'], ['false'], ['true'], ['unverified']
    #y_train = []
    #print('label',label,'\n')
...
def constructTree(tree):
    ...
    for j in tree:
        ...
        ## not root node ## 
        if not indexP == 'None':
            if int(indexP) not in index2node.keys():   
                continue
...
def loadData():
    ...
    for eid in open(trainPath):
        #if c > 8: break
        eid = eid.rstrip()
        if eid not in labelDic.keys(): continue
        if eid not in treeDic.keys(): continue 
